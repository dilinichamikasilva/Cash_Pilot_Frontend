...
export default function UpdateSpendingPage() {
        ...
        <div className="flex-1 w-0 p-4">
          <div className="flex items-start">
            <div className="flex-shrink-0 pt-0.5">
              ...
              {type === 'error' && <AlertTriangle className="h-10 w-10 text-rose-500" />}
              {type === 'info' && <Loader2 className="h-10 w-10 text-indigo-500 animate-spin" />}
            </div>
            <div className="ml-3 flex-1">
              <p className="text-sm font-bold text-slate-900">
                {type === 'success' ? 'Transaction Logged' : type === 'error' ? 'Action Failed' : 'Update'}
              </p>
  ...
  const fetchBudget = async () => {
    try {
      ...
      setCategories([]); 
      showToast("Failed to fetch budget categories.", "error");
    }
  };
  ...
  const handleSubmitTransaction = async (e: React.FormEvent) => {
    ...
    try {
      await addTransaction(formData);
      showToast(`Successfully added Rs. ${Number(amount).toLocaleString()} to ${selectedCat.name}`, "success");
      ...
      showToast("Could not upload transaction. Please try again.", "error");
    } finally { 
      setUploading(false); 
    }
  };
          ...
          <div className="space-y-6">
            ...
            {categories.length === 0 && (
              <div className="bg-white rounded-[2.5rem] border border-slate-100 p-20 text-center">
                ...
                <p className="text-slate-400 font-bold">No categories found for this period.</p>
              </div>
            )}
          </div>
