                ...
                stats.categories.slice(0, 4).map((cat) => (
                  <div key={cat.id}>
                    <div className="flex justify-between text-sm font-bold mb-2">
                      ...
                      <span className="text-slate-900">{stats.currency} {cat.spent.toLocaleString()}</span>
                    </div>
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: `${cat.budget > 0 ? Math.min((cat.spent / cat.budget) * 100, 100) : 0}%` }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {stats.categories.filter(c => c.spent > 0).slice(0, 4).map((cat) => (
                <div key={cat.id} className="bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-colors group">
                  ...
                  <p className="font-black text-rose-400">- {stats.currency} {cat.spent.toLocaleString()}</p>
                </div>
              ))}
            </div>
          </div>
